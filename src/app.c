...
// Forward declaration for HX711 sensor interface
extern bool cc_multilevel_sensor_endpoint_0_weight_interface_read_value(sensor_read_result_t* o_result, uint8_t i_scale);
...
zaf_event_distributor_app_event_manager(const uint8_t event)
{
  ...
  switch (event) {
    case EVENT_APP_SEND_BATTERY_LEVEL_AND_SENSOR_REPORT:
      ZPAL_LOG_DEBUG(ZPAL_LOG_APP, ">>> Sending battery report\n");
      (void) CC_Battery_LevelReport_tx(NULL, ENDPOINT_ROOT, NULL);
      ZPAL_LOG_DEBUG(ZPAL_LOG_APP, ">>> Calling cc_multilevel_sensor_send_sensor_data()\n");
      cc_multilevel_sensor_send_sensor_data();
      ZPAL_LOG_DEBUG(ZPAL_LOG_APP, ">>> Sensor send completed\n");

      // Direct test of HX711 sensor (for development/debug)
      ZPAL_LOG_DEBUG(ZPAL_LOG_APP, ">>> DIRECT TEST: Calling HX711 read function\n");
      sensor_read_result_t result;
      bool success = cc_multilevel_sensor_endpoint_0_weight_interface_read_value(&result, 0);
      if (success) {
        ZPAL_LOG_INFO(ZPAL_LOG_APP, ">>> DIRECT TEST: HX711 read SUCCESS!\n");
      } else {
        ZPAL_LOG_ERROR(ZPAL_LOG_APP, ">>> DIRECT TEST: HX711 read FAILED!\n");
      }
      break;
    default:
      break;
